...
function runOpenSeek({ input, expectedIds, openseekBin, timeoutMs, temporaryRoot, environment, onChild }) {
  return new Promise(resolveRun => {
    ...
    let skillsPath;
    try {
      runDirectory = mkdtempSync(join(temporaryRoot, "ldiff-analysis-"));
...
export function createLdiffServer(options = {}) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const server = createLdiffServer();
  server.listen(port, host, () => {
    process.stdout.write(`ldiff playground listening on http://${host}:${port}\n`);
  });
}
